...
class FarmerMainWindow(QMainWindow):
    ...
    def view_sales_analytics(self):
        try:
            ...
            self.display_analytics("Sales Analytics", rows, cursor.description)
        except Exception as e:
            self.show_error(str(e))
    ...
    def view_inventory_analytics(self):
        try:
            ...
            self.display_analytics("Inventory Analytics", rows, column_descriptions)
        except Exception as e:
            self.show_error(str(e))
    ...
    def view_seasonal_patterns(self):
        try:
            ...
            rows = cursor.fetchall()
            column_descriptions = cursor.description
            self.display_analytics("Seasonal Patterns", rows, column_descriptions)
        except Exception as e:
            self.show_error(str(e))
    ...
    def forecast_demand(self):
        try:
            ...
            rows = cursor.fetchall()
            column_descriptions = cursor.description
            self.display_analytics("Demand Forecast", rows, column_descriptions)
        except Exception as e:
            self.show_error(str(e))

    def display_analytics(self, title, rows, column_descriptions):
        while self.right_layout.count():
            item = self.right_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
        ...
        table_widget = QTableWidget()
        table_widget.setRowCount(len(rows))
        table_widget.setColumnCount(len(column_descriptions))
        table_widget.setHorizontalHeaderLabels([desc[0] for desc in column_descriptions])
